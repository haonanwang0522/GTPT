...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, now_epoch, now_step):
    total_iters = epochs * niter_per_ep
    now_iters = now_epoch * niter_per_ep + now_step
    schedule = final_value + 0.5 * (base_value - final_value) * (1 + math.cos(math.pi * now_iters / total_iters))
    return schedule
...
def train_sa_simdr(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target_x, target_y, target_weight, meta) in enumerate(train_loader):
        ...
        loss = criterion(output, target_x, target_y, target_weight)
        ...
        # EMA
        if hasattr(model.module, 'ema_features'):
            with torch.no_grad():
                m = cosine_scheduler(0.996, 1, 300, len(train_loader), epoch, i)  # momentum parameter
                # m = 0.997

                model_state = model.module.features.state_dict()
                ema_model_state = model.module.ema_features.state_dict()
                for name in ema_model_state:
                    ema_model_state[name] = m * ema_model_state[name] + (1-m) * model_state[name]
                model.module.ema_features.load_state_dict(ema_model_state, strict=True)


                model_state = model.module.mlp_head_x.state_dict()
                ema_model_state = model.module.ema_mlp_head_x.state_dict()
                for name in ema_model_state:
                    ema_model_state[name] = m * ema_model_state[name] + (1-m) * model_state[name]
                model.module.ema_mlp_head_x.load_state_dict(ema_model_state, strict=True)

                model_state = model.module.mlp_head_y.state_dict()
                ema_model_state = model.module.ema_mlp_head_y.state_dict()
                for name in ema_model_state:
                    ema_model_state[name] = m * ema_model_state[name] + (1-m) * model_state[name]
                model.module.ema_mlp_head_y.load_state_dict(ema_model_state, strict=True)
            
                # loss = criterion(ema_x, ema_y, target_x, target_y, target_weight)
