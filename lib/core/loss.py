...
class KLDiscretGlobalPerceivedLoss(nn.Module):
    def __init__(self, *arg, **kwargs):
        super(KLDiscretGlobalPerceivedLoss, self).__init__()
        self.LogSoftmax = nn.LogSoftmax(dim=1) #[B,LOGITS]
        self.criterion_ = nn.KLDivLoss(reduction='none')
        
    def criterion(self, dec_outs, labels, log_target=False):
        if log_target:
            T = 1
            return F.kl_div(
                        F.log_softmax(dec_outs / T, dim=1),
                        F.log_softmax(labels / T, dim=1),
                        reduction='none',
                        log_target=True
                    ).mean(1) * T ** 2
        scores = self.LogSoftmax(dec_outs)
        loss = self.criterion_(scores, labels).mean(1) 
        return loss

    def forward(self, outputs, target_x, target_y, target_weight):
        output_x = outputs['pred_x']
        output_y = outputs['pred_y']
        entire_x = outputs['entire_pred_x'] if 'entire_pred_x' in outputs else None
        entire_y = outputs['entire_pred_y'] if 'entire_pred_y' in outputs else None
        ...
        for idx in range(num_joints):
            ...
            coord_x_gt = target_x[:,idx].squeeze()
            coord_y_gt = target_y[:,idx].squeeze()
            if entire_x is not None:
                coord_x_pred = entire_x[:,idx].squeeze()
                coord_y_pred = entire_y[:,idx].squeeze()
                loss += (self.criterion(coord_x_pred,coord_x_gt).mul(weight).mean()) 
                loss += (self.criterion(coord_y_pred,coord_y_gt).mul(weight).mean())
            coord_x_pred = output_x[:,idx].squeeze()
            coord_y_pred = output_y[:,idx].squeeze()
            ...
            if entire_x is not None:
                coord_x_gt = entire_x[:,idx].squeeze().detach()
                coord_y_gt = entire_y[:,idx].squeeze().detach()
                loss += (self.criterion(coord_x_pred,coord_x_gt, True).mul(weight).mean()) 
                loss += (self.criterion(coord_y_pred,coord_y_gt, True).mul(weight).mean())
        return loss / num_joints 
