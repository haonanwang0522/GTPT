...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.down_layer1 = self._make_layer(256, 4)
        self.down_layer2 = self._make_layer(384, 4)
        self.down_layer3 = self._make_layer(512, 4)
        self.upsample1 = self._make_upsample(512,384)
        self.upsample2 = self._make_upsample(384,256)
        self.up_layer1 = self._make_up_layer(384, 2)
        self.up_layer2 = self._make_up_layer(256, 2)
    ...
    def _make_up_layer(self, out_channels, num_blocks):
    ...
    def forward(self, x):
        ...
        x2 = self.up_layer1(torch.cat([x2, self.upsample1(x3)], 1))
        x1 = self.up_layer2(torch.cat([x1, self.upsample2(x2)], 1))
        return x1
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None, pos = None, sparse_keypoint_token = None, dense_keypoint_token = None, keep_rate = None):
        K = 1
        B, N, C = x.shape
        N_img = N - K
        ...
        part_attn, kp_attn = attn_logit[:,:,0:5,K:], attn_logit[:,:,5:K,K:]
        global_attn = [
            torch.cat([part_attn[:,:,0:1], kp_attn[:,:,0:5]], 2),
            torch.cat([part_attn[:,:,1:3], kp_attn[:,:,5:11]], 2),
            torch.cat([part_attn[:,:,3:5], kp_attn[:,:,11:17]], 2)
        ]
        global_attn = [self.softmax_pool(a.flatten(1,2),1) for a in global_attn]
        now_attn = torch.stack(global_attn, 1) # [B G N]
        sorted_idx = torch.argsort(now_attn, dim=2, descending=True)
        img = torch.gather(img, 2, sorted_idx[..., None].repeat(1,1,1,C))
        pos = torch.gather(pos, 2, sorted_idx[..., None].repeat(1,1,1,C))
        
        part_face = part[:,0:1].repeat(1,68,1) + dense_keypoint_token[:,0:68]
        part_upper = torch.cat([part[:,1:2].repeat(1,21,1), part[:,2:3].repeat(1,21,1)], 1) + dense_keypoint_token[:,68:110]
        part_lower = torch.cat([part[:,3:4].repeat(1,3,1) , part[:,4:5].repeat(1,3,1)] , 1) + dense_keypoint_token[:,110:116]

        img_face = torch.cat([part_face, img[:,0,:-68-(now_img_token-tgt_img_token)]], 1)
        img_upper = torch.cat([part_upper, img[:,1,:-42-(now_img_token-tgt_img_token)]], 1)
        img_lower = torch.cat([part_lower, img[:,2,:-6-(now_img_token-tgt_img_token)]], 1)
        img = torch.stack([img_face, img_upper, img_lower], 1)
        pos_face = torch.cat([torch.zeros_like(part_face), pos[:,0,:-68-(now_img_token-tgt_img_token)]], 1)
        pos_upper = torch.cat([torch.zeros_like(part_upper), pos[:,1,:-42-(now_img_token-tgt_img_token)]], 1)
        pos_lower = torch.cat([torch.zeros_like(part_lower), pos[:,2,:-6-(now_img_token-tgt_img_token)]], 1)
        pos = torch.stack([pos_face, pos_upper, pos_lower], 1)
        ...
        for idx,(attn, ff) in enumerate(self.group_layers):
            ...
            if now_img_token != tgt_img_token:
                ...
                group_x, group_pos = [], []
                idx1 = 0
                dense_num = dense_nums[idx1]
                global_attn = attn_logit[:,:,idx1,:K+dense_num,3*K+dense_num:] # [B H K N]
                kp_token, img_token = x[:,idx1,:K+dense_num], x[:,idx1,K+dense_num:]          # [B K C] [B N C]

                now_attn = self.softmax_pool(global_attn.flatten(1,2),1)
                _, sorted_idx = now_attn.topk(tgt_img_token-dense_num, dim=1)
                pruned_img_token = torch.gather(img_token, 1, sorted_idx[..., None].repeat(1,1,C))
                pruned_pos = torch.gather(pos[:,idx1,dense_num:], 1, sorted_idx[..., None].repeat(1,1,C))
                group_x.append(torch.cat([kp_token, pruned_img_token], 1))
                group_pos.append(torch.cat([pos[:,idx1,:dense_num], pruned_pos], 1))
                ...
                x = torch.stack(group_x, 1)
                pos = torch.stack(group_pos, 1)
        ...
        kp_hand = x[:,1,K:K+42]
        kp_foot = x[:,2,K:K+6]
        return torch.cat([kp_body, kp_foot, kp_face, kp_hand], 1), attns
...
class TokenPose_S_base(nn.Module):
    def __init__(self, *, image_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, channels = 3, dropout = 0., emb_dropout = 0.,pos_embedding_type="learnable"):
        ...
        self.keypoint_token = nn.Parameter(torch.zeros(1, 17, dim))
        self.extra_token = nn.Parameter(torch.zeros(1, 5, dim))
        self.fine_keypoint_token = nn.Parameter(torch.zeros(1, self.num_keypoints - 17, dim))
        ...
        trunc_normal_(self.fine_keypoint_token, std=.02)
        if apply_init:
    ...
    def forward(self, img):
        ...
        sparse_keypoint_tokens = repeat(torch.cat([self.extra_token, self.keypoint_token], 1), '() n d -> b n d', b = b)
        dense_keypoint_tokens = repeat(self.fine_keypoint_token, '() n d -> b n d', b = b)
        x += self.pos_embedding[:, :n]
        x = torch.cat((human_token, x), dim=1)
        x = self.dropout(x)
        return x, sparse_keypoint_tokens, dense_keypoint_tokens

    def trans(self, x, sparse_keypoint_tokens, dense_keypoint_tokens, mask = None, keep_rate = None):
        x, attns = self.transformer(x, mask,self.pos_embedding, sparse_keypoint_tokens, dense_keypoint_tokens, keep_rate)
        return x, attns   
...
class TokenPose_S(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.features = TokenPose_S_base(
                            ...
                            num_keypoints = cfg.MODEL.NUM_JOINTS, dim = cfg.MODEL.DIM, channels = 256,
    ...
    def forward(self, img):
        ...
        if self.training:
            feat, sparse_kp_tokens, dense_kp_tokens = self.features(img)
            x, _ = self.features.trans(feat, sparse_kp_tokens, dense_kp_tokens, keep_rate=stu_keep_rate)
            outputs['pred_x'] = self.mlp_head_x(x)
            outputs['pred_y'] = self.mlp_head_y(x)
            tch_keep_rate = [1 for _ in range(12)]
            x, _ = self.features.trans(feat, sparse_kp_tokens, dense_kp_tokens, keep_rate=tch_keep_rate)
            outputs['entire_pred_x'] = self.mlp_head_x(x)
            outputs['entire_pred_y'] = self.mlp_head_y(x)
            return outputs
        with torch.no_grad():
            feat, sparse_kp_tokens, dense_kp_tokens = self.ema_features(img)
            x, _ = self.ema_features.trans(feat, sparse_kp_tokens, dense_kp_tokens, keep_rate=stu_keep_rate)
